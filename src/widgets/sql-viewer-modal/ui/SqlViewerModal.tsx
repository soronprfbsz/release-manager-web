...
export function SqlViewerModal({ open, onOpenChange, fileId, fileName }: SqlViewerModalProps) {
  const [copied, setCopied] = useState(false)
  const { toast } = useToast()

  const { data: sqlContent, isLoading, error } = useQuery({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col gap-4">
        <DialogHeader>
          ...
          <DialogDescription>SQL 파일 내용</DialogDescription>
        </DialogHeader>
